...
import {
    X, Loader2, Brain, Microscope, Lightbulb, Wrench,
    BarChart3, Zap, Database, Play, Search, Compass
} from 'lucide-react';
...
// We'll use a placeholder for environment since it might vary
// import { environment } from '../../config/environment'; 
const environment = { geminiApiKey: "placeholder-key" };
...
const AdvancedAgentsPopup: React.FC<AdvancedAgentsPopupProps> = ({ isOpen, onClose, content }) => {
    ...
    const agents: Agent[] = (AGENT_CONFIGS as any[]).map((config: AIAgentConfig) => ({
        ...
        capabilities: (() => {
            ...
            if (Array.isArray(config.capabilities)) {
                return {
                    ...
                    ...Object.fromEntries(config.capabilities.map((cap: string) => [cap, true]))
                };
            }
                                    ...
                                    {filteredAgents.map((agent) => (
                                        ...
                                        >
                                            ...
                                            <button
                                                ...
                                                className={`mt-4 w-full py-2 px-4 rounded-lg flex items-center justify-center ${isAnalyzing && selectedAgent?.id === agent.id
